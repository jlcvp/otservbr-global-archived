...
function NetworkMessage.create(pdata)
	...
	-- @params
	--	'pdata': string
	-- Instancia a classe NetworkMessage, se pdata � fornecido � definido o atributo data igual a pdata e size igual a #pdata
...
function NetworkMessage:getRanges(byteCount, signed)
	...
	-- 	'byteCount': number
	--  'signed': boolean
	-- Fun��o interna usada para calcular o limite do valor de 'byteCount' bytes, se signed � true reajusta seu valor para signed
...
function NetworkMessage:readBytes(byteCount, signed)
	...
	-- se possivel a leitura, e false se n�o. Se signed � true reajusta valor para signed
...
function NetworkMessage:addBytes(value, count, signed)
	...
	-- suporta retorna false. Se signed � true reajusta valor para signed
	if signed then
		value = value * 2
	end
...
desc = string.format('{')
local last = 0
local filename = "patch\\store\\staticdata.dat"

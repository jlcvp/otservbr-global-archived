...
local config = {
	...
	itens = {
		[2798] = {price = 50},
	    [20620] = {price = 300},
	    [33082] = {price = 50}, -- exercises bg
	    [33083] = {price = 50},
	    [33084] = {price = 50},
	    [33085] = {price = 50},
	    [33086] = {price = 50},
	    [33087] = {price = 50}, -- exercises end
	    [23588] = {price = 150},
	    [16007] = {price = 1500},
	    [27063] = {price = 1500},
		[3954] = {price = 250},
	...
	outfits = {
		['lion of war'] = {price = 2600, idF = 1206, idM = 1207},
		['veteran paladin'] = {price = 2600, idF = 1204, idM = 1205},
		['void master'] = {price = 2600, idF = 1202, idM = 1203}
	},
	mounts = {
		[44] = {price = 2600},
		[45] = {price = 2600},
	...
	otherStuff = {
		["all bless"] = {price = 30},
		["boost exp"] = {price = 150},
		["prey bonus reroll"] = {price = 100},
		["prey bonus reroll 5x"] = {price = 450},
		["instant reward access"] = {price = 50},
		["instant reward access 10x"] = {price = 400},
	}
}
...
local function buyOfferStuff(playerid, price, itemName)
	local player = Player(playerid)
	if not player then
		return CONST_RET_STUFF.NONE
	end

	if player:getItemCount(moeda) < price then
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif npcHandler.topic[cid] == 3 then
		...
		if itensTable then
			price[cid] = itensTable.price
			itemName[cid] = msg
			npcHandler:say({"Good choice! If you really want the ".. itemName[cid] ..", it will cost you ".. price[cid] .." ".. ItemType(moeda):getName():lower() .." each. How many would you be interested in?"}, cid)
	...
	elseif npcHandler.topic[cid] == 4 then
		count[cid] = tonumber(msg)
		if count[cid] then	
			if count[cid] > 1 then
				plural = plural .. "s"
			end
			if count[cid] >= 20 then
				npcHandler:say({"Calm down!! It doesn't fit in a bag!"}, cid)
			else
				totalPrice[cid] = price[cid]*count[cid]
				npcHandler:say({"So you want ".. count[cid] .." ".. itemName[cid] .. plural ..". Hmmmm, this will give a total of ".. totalPrice[cid] .." ".. ItemType(moeda):getName():lower() .."s. Do you accept?"}, cid)
				playerTopic[cid] = 5
	...
	elseif npcHandler.topic[cid] == 5 then
		if msgcontains(msg, "yes") or msgcontains(msg, "sim") then
			if player:getItemCount(moeda) >= totalPrice[cid] then					
				itemId[cid] = ItemType(itemName[cid]):getId()
				-- Criando container no mailbox
				local parcel = player:getInbox():addItem(2596, 1, false, 1)
				parcel:addItem(1988, 1, false, 1):addItem(itemId[cid], count[cid], false, 1)
				player:removeItem(moeda, totalPrice[cid]) 
	...
	elseif npcHandler.topic[cid] == 7 then
		...
		if mountTable then
			price[cid] = mountTable.price
			itemName[cid] = msg
			npcHandler:say({"Good choice! If you really want the ".. itemName[cid] ..", it will cost you ".. price[cid] .." ".. ItemType(moeda):getName():lower() .."s. Are you sure?"}, cid)
	...
	elseif npcHandler.topic[cid] == 8 then
		if msgcontains(msg, "yes") or msgcontains(msg, "sim") then
			if player:getItemCount(moeda) >= price[cid] then
				if player:hasMount(itemName[cid]) then
					...
					player:removeItem(moeda, price[cid])
	...
	elseif npcHandler.topic[cid] == 10 then
		...
		if outfitTable then
			outfit.male = outfitTable.idM
			outfit.female = outfitTable.idF
			price[cid] = outfitTable.price
			itemName[cid] = msg
			npcHandler:say({"Good choice! If you really want the ".. itemName[cid] ..", it will cost you ".. price[cid] .." ".. ItemType(moeda):getName():lower() .."s. Are you sure?"}, cid)
	...
	elseif npcHandler.topic[cid] == 11 then
		if msgcontains(msg, "yes") or msgcontains(msg, "sim") then
			if player:getItemCount(moeda) >= price[cid] then
				if player:hasOutfit(outfit.male or outfit.female) then
	...
	elseif npcHandler.topic[cid] == 13 then
		...
		if otherStuffTable then
			price[cid] = otherStuffTable.price
			itemName[cid] = msg
			npcHandler:say({"Good choice! If you really want the ".. itemName[cid] ..", it will cost you ".. price[cid] .." ".. ItemType(moeda):getName():lower() .."s. Are you sure?"}, cid)
	...
	elseif npcHandler.topic[cid] == 14 then
		if msgcontains(msg, "yes") or msgcontains(msg, "sim") then
			if player:getItemCount(moeda) >= price[cid] then
				local ret = buyOfferStuff(player:getId(), price[cid], itemName[cid])
